...
//						k++;
				...
				else if(i==len-2 && j==len-1 && str[i]==str[j])
				{
					k=k-2;
				}
			} 	
			else
			{
				...
				{
					k--;
					printf("str[i]!=str[j]  k- %d\n",k);
				}
				printf("i-%d j-%d len-%d\n",i,j,len);
				if(i!=1 && str[i]==str[j] && i<=len-3 && j<=len-2)
				{
					k++;
					printf("i!=1 && str[i]==str[j] && i<=len-3 && j<=len-2  k-%d\n",k);

				}
				if(i==len-2 && j==len-1 && str[i]==str[j])
				{
					k=k-2;
					printf("i==len-2 && j==len-1 && str[i]==str[j]  k-%d\n",k);
